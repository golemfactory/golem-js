...
import { NetworkApiMock, setExpectedError, clear } from "../mock/rest/network";
...
describe("Network", () => {
  ...
  describe("Creating", () => {
    it("should create network", async () => {
      const network = await Network.create({ ownerId: "test_owner_id" });
    ...
    it("should create network with 16 bit mask", async () => {
      const network = await Network.create({ ownerId: "1", ip: "192.168.7.0/16" });
    ...
    it("should not create network without mask", async () => {
      const shouldFail = Network.create({ ownerId: "1", ip: "1.1.1.1" });
      await expect(shouldFail).to.be.rejectedWith(Error, "Cidr notation should be in the form [ip number]/[range]");
    });
  ...
  describe("Nodes", () => {
    ...
    it("should not add too many nodes", async () => {
      const network = await Network.create({ ownerId: "1", ip: "192.168.0.0/30" });
    ...
    it("should get node websocket uri", async () => {
      ...
      expect(node.getWebsocketUri(22)).to.deep.equal(
        `ws://127.0.0.1:7465/net-api/v1/net/${network.id}/tcp/192.168.0.2/22`
      );
    });
  });
