...
describe("Work Context", () => {
  ...
  describe("Executing", () => {
    ...
    it("should execute upload file command", async () => {
      const activity = await Activity.create("test_agreement_id", yagnaApi);
      const worker: Worker<Result> = async (ctx) => ctx.uploadFile("./file.txt", "/golem/file.txt");
    ...
    it("should execute upload json command", async () => {
      const activity = await Activity.create("test_agreement_id", yagnaApi);
      const worker: Worker<Result> = async (ctx) => ctx.uploadJson({ test: true }, "/golem/file.txt");
      const ctx = new WorkContext(activity, {
        logger,
        activityStateCheckingInterval: 10,
        storageProvider: storageProviderMock,
    ...
    it("should execute download file command", async () => {
      const activity = await Activity.create("test_agreement_id", yagnaApi);
      const worker: Worker<Result> = async (ctx) => ctx.downloadFile("/golem/file.txt", "./file.txt");
      const ctx = new WorkContext(activity, {
        logger,
        activityStateCheckingInterval: 10,
        storageProvider: storageProviderMock,
  ...
  describe("Batch", () => {
    it("should execute batch as promise", async () => {
      const activity = await Activity.create("test_agreement_id", yagnaApi);
      const worker: Worker<Result[]> = async (ctx) => {
        return ctx
          .beginBatch()
    ...
    it("should execute batch as stream", async () => {
      const activity = await Activity.create("test_agreement_id", yagnaApi);
      const worker: Worker<Readable> = async (ctx) => {
        return ctx
          .beginBatch()
          .run("some_shell_command")
          .uploadFile("./file.txt", "/golem/file.txt")
  ...
  describe("Error handling", () => {
    it("should return a result with error in case the command to execute is invalid", async () => {
      const activity = await Activity.create("test_agreement_id", yagnaApi);
      const worker: Worker<Result[]> = async (ctx) => ctx.beginBatch().run("invalid_shell_command").end();
      const ctx = new WorkContext(activity, {
        logger,
        activityStateCheckingInterval: 10,
        storageProvider: storageProviderMock,
    ...
    it("should catch error while executing batch as stream with invalid command", async () => {
      const activity = await Activity.create("test_agreement_id", yagnaApi);
      const worker: Worker<Readable> = async (ctx) => ctx.beginBatch().run("invalid_shell_command").endStream();
      const ctx = new WorkContext(activity, {
        logger,
        activityStateCheckingInterval: 10,
        storageProvider: storageProviderMock,
