...
function _find_enum(enum_type: object, val: string): any {
  for (const member of Object.entries(enum_type)) {
...
export class Field {
  ...
  private _name;
  private _type;
  constructor({ value, metadata, name }: { value?: any; metadata?: object; name?: string; type?: object } = {}) {
...
class _PyField {
  ...
  encode(value: string) {
    if (this.type === Date) return [this.name, dayjs.unix(parseInt((parseFloat(value) * 0.001).toString())).toDate()];
...
export class Model {
  ...
  fields(cls): Field[] {
    const fields: Field[] = [];
    const props = getAllProperties(cls);
  ...
  from_properties(props: object): any {
    const field_map = {};
    const data = {};
    for (const f of this.fields(this)) {
    ...
    for (const [key, val] of Object.entries(props)) {
      if (key in field_map) {
        const [_key, _val] = field_map[key].encode(val);
        data[_key] = _val;
      }
    }

    this._custom_mapping(props, data);
    const self = new (Object.getPrototypeOf(this).constructor)();
