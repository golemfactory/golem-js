...
export default class AsyncWrapper {
  ...
  constructor(wrapped: Callable<any, any>, event_loop: any = null, cancellationToken) {
  ...
  async ready() {
    this._task = this._loop.create_task(this._worker.bind(this)).catch(() => null); // the task might be cancelled
  }
