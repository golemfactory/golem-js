...
export class AgreementConfigContainer {
  ...
  constructor(readonly options: AgreementOptions, readonly eventBus: EventBus, readonly logger?: Logger) {
    const apiKey = this.options?.yagnaOptions?.apiKey || process.env.YAGNA_APPKEY;
    const basePath = this.options?.yagnaOptions?.basePath || process.env.YAGNA_API_BASEPATH;
    if (!apiKey) throw new Error("Api key not defined");
    if (!basePath) throw new Error("Api base path not defined");
    const apiConfig = new yaMarket.Configuration({
      apiKey,
