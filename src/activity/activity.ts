      ...
      async read() {
        while (!isBatchFinished) {
          ...
          if (startTime.valueOf() + (timeout || activityExecuteTimeout) <= new Date().valueOf()) {
            logger?.debug("Activity probably timed-out, will terminate");
            return this.destroy(new Error(`Activity ${activityId} timeout.`));
          }

          if (!isRunning()) {
            logger?.debug("Activity is no longer running, will terminate");
            return this.destroy(new Error(`Activity ${activityId} has been interrupted.`));
          }

          try {
            logger?.debug("Trying to get the results from the API");
            ...
            logger?.debug(`Received the following results: ${JSON.stringify(newResults)}`);
            ...
            logger?.error("Processing results received from the API failed");
